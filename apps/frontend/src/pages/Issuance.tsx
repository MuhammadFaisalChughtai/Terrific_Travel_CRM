...
export default function IssuancePage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (ticketId: string) => {
      const res = await apiClient.delete(`/issuance/tickets/${ticketId}`);
      return res.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['issuance-tickets'] });
      toast.success('Issuance request removed from the board');
      setViewTicket(null);
    },
    onError: (err: any) => {
      const msg = err.response?.data?.error || err.response?.data?.message || err.message || 'Failed to remove issuance request';
      toast.error(msg);
    },
  });
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                if (
                                  window.confirm(
                                    `Are you sure you want to remove issuance request ${ticket.ticketNumber} (${ticket.leadGuestName}) from the board?`
                                  )
                                ) {
                              ...
                              className="p-1 rounded text-muted-foreground hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30 opacity-40 group-hover:opacity-100 transition-all cursor-pointer"
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-card rounded-2xl max-w-xl w-full p-6 shadow-2xl border border-border max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateSubmit} className="mt-4 space-y-4">
              ...
              {/* Lead Guest & Contact */}
              <div className={`grid grid-cols-1 ${newType === 'FLIGHT' ? 'md:grid-cols-2' : ''} gap-3`}>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1">
                    Lead Guest Name *
                  </label>
            ...
            <div className="mt-5 flex items-center justify-between">
              <button
                ...
                onClick={() => {
                  if (
                    window.confirm(
                      `Are you sure you want to remove issuance request ${viewTicket.ticketNumber} (${viewTicket.leadGuestName}) from the board?`
                    )
                  ) {
                    deleteMutation.mutate(viewTicket.id);
                  }
                }}
                disabled={deleteMutation.isPending}
