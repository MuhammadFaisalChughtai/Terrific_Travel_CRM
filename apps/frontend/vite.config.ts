...
export default defineConfig(({ mode }) => {
  ...
  const backendUrl = process.env.VITE_BACKEND_URL || rootEnv.VITE_BACKEND_URL || env.VITE_BACKEND_URL || 'http://localhost:3000';

  const currency       = process.env.CURRENCY        || rootEnv.CURRENCY        || env.VITE_CURRENCY        || 'USD';
  const currencySymbol = process.env.CURRENCY_SYMBOL || rootEnv.CURRENCY_SYMBOL || env.VITE_CURRENCY_SYMBOL || '';
